...
} from '@/Services/Modules/members/membersApi'
...
export default function AdminMembersContainer() {
  ...
  const handleToggleVerify = async (member: Member) => {
    ...
    try {
      ...
      const res = await updateStatus({ idnumber: member.IDNumber, action }).unwrap()
      showNotification({ title: 'Berhasil', description: res.message, type: 'success' })
    } catch (error: any) {
      showNotification({
  ...
  const handleDelete = async (member: Member) => {
    if (!window.confirm(`Hapus peserta "${member.Name}"? Tindakan ini tidak bisa dibatalkan.`)) return
    setDeletingId(member.IDNumber)
    try {
      const res = await deleteMember(member.IDNumber).unwrap()
      showNotification({ title: 'Berhasil', description: res.message, type: 'success' })
      setSelectedIds((prev) => prev.filter((id) => id !== member.IDNumber))
    } catch (error: any) {
      showNotification({
  ...
  const runBulkAction = async (action: 'verifikasi' | 'batalkan_verifikasi' | 'hapus') => {
    ...
    if (action === 'hapus' && !window.confirm(`Hapus ${selectedIds.length} peserta terpilih?`)) return

    try {
      const res = await bulkAction({ selected_ids: selectedIds, action }).unwrap()
      showNotification({ title: 'Berhasil', description: res.message, type: 'success' })
      clearSelection()
    } catch (error: any) {
      showNotification({
  ...
  return (
    <AppLayout title="Daftar Peserta" subtitle="Kelola data peserta, verifikasi akun, dan hapus data.">
